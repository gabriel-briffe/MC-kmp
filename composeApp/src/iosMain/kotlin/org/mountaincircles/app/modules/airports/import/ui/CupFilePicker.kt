...
/**
 * iOS implementation of CupFilePicker
 * Uses the global CupFilePickerManager that's initialized in the iOS app
 */
actual class CupFilePicker {
    ...
    actual suspend fun launchCupSave(module: AirportsModule, onResult: suspend (Boolean) -> Unit) = suspendCancellableCoroutine<Unit> { continuation ->
        try {
            val manager = org.mountaincircles.app.modules.airports.getCupFilePickerManager()
            if (manager != null) {
