...
private const val TAG = "GlanceWidget_WaveWorker"

/**
 * WorkManager worker for Wave Today Glance widget updates.
 * 
 * Completely decoupled from the legacy WaveForecastWorker.
 * Only updates TODAY's wave forecast for the Glance widget.
 */
class GlanceWaveForecastWorker(
    context: Context,
    ...
    override suspend fun doWork(): Result {
        return try {
            Logger.log(TAG, LogLevel.INFO, "Starting Glance wave forecast update...")

            // Get stored tile metadata
            ...
            // Perform the wave forecast update for TODAY only
            performTodayWaveForecastUpdate(metadata)
    ...
    private suspend fun performTodayWaveForecastUpdate(metadata: WidgetTileMetadata): Result {
        Logger.log(TAG, LogLevel.INFO, "Processing TODAY wave forecast...")
        ...
        // Download and process TODAY's wave forecast
        val todayResult = downloadAndExtractWaveForecast(metadata, isTomorrow = false)
        if (todayResult.success) {
            val compositedBitmap = compositeWaveOverTerrain(terrainBitmap, todayResult.bitmap)
            saveCompositedImage(applicationContext, compositedBitmap, "widget_today_overlay.png")
            markWorkDoneToday(applicationContext)
            Logger.log(TAG, LogLevel.INFO, "TODAY wave forecast updated successfully")
        } else {
            // Delete old overlay so widget shows terrain-only with "no wave data yet"
            deleteOverlayImage(applicationContext, "widget_today_overlay.png")
            Logger.log(TAG, LogLevel.WARN, "TODAY download failed - removed old overlay")
        }

        // Mark refresh as complete (this also saves timestamp)
        WaveTodayRepo.markRefreshComplete(applicationContext)

        // Reload image data and update the Glance widget
        WaveTodayRepo.loadData(applicationContext)
        WaveTodayGlanceWidget().updateAll(applicationContext)
    ...
    private fun shouldPerformUpdate(context: Context): Boolean {
        ...
        val hasFile = File(context.filesDir, "widget_images/widget_today_overlay.png").exists()
        return newDay || !hasFile
    }

    private fun hasWorkBeenDoneToday(context: Context): Boolean {
        return try {
            val prefs = context.getSharedPreferences("glance_wave_today_prefs", Context.MODE_PRIVATE)
    ...
    companion object {
        private const val WORK_NAME = "glance_wave_today_worker"

        /**
         * Schedule daily wave forecast updates for the Glance widget.
         */
        fun scheduleDaily(context: Context) {
            val workRequest = PeriodicWorkRequestBuilder<GlanceWaveForecastWorker>(
                24, TimeUnit.HOURS,
                2, TimeUnit.HOURS
            )
            ...
            Logger.log(TAG, LogLevel.INFO, "Scheduled daily Glance wave updates")
        }

        /**
         * Run wave forecast update immediately.
         */
        fun runImmediate(context: Context) {
            val workRequest = OneTimeWorkRequestBuilder<GlanceWaveForecastWorker>()
                .build()
            WorkManager.getInstance(context).enqueue(workRequest)
            Logger.log(TAG, LogLevel.INFO, "Scheduled immediate Glance wave update")
        }

        /**
         * Run wave forecast update immediately with force refresh.
         */
        fun runImmediateForceRefresh(context: Context) {
            val workRequest = OneTimeWorkRequestBuilder<GlanceWaveForecastWorker>()
                ...
                .build()
            WorkManager.getInstance(context).enqueue(workRequest)
            Logger.log(TAG, LogLevel.INFO, "Scheduled forced Glance wave update")
        }
