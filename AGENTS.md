...
- **Offline pack style:** `BasemapStyle.buildOfflinePackJson()` (written to `filesDir/offline/basemap-style.json`) omits empty `sprite` / `glyphs` entries that can stall offline enumeration. Live map still uses `BasemapStyle.buildJson(glyphsBaseUri)`.
