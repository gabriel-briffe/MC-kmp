...
- **Basemap style URL:** Live map and offline packs both use [BasemapStyle.SHARED_STYLE_URL] (`https://raw.githubusercontent.com/gabriel-briffe/MC-kmp/main/styles/basemap-raster-osm.json`). Keep `styles/basemap-raster-osm.json` in sync on `main`. Mapterhorn hillshade is a Compose layer on the live map only.
