...
- **Offline pack style:** `BasemapStyle.buildOfflinePackJson()` (written to `filesDir/offline/basemap-style.json`) is **OSM raster only** (z0–7). MapLibre offline pyramids do not reliably prefetch `raster-dem`/hillshade; live map still uses full `buildJson()` with Mapterhorn. Empty `sprite` / `glyphs` are omitted from the offline style.
